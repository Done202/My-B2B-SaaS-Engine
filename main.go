...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		// এক্সপোর্ট লজিক: রেঞ্জ এবং কাস্টম হ্যান্ডলিং
		if action == "export_excel" || action == "export_pdf" {
			format := "EXCEL"
			if action == "export_pdf" { format = "PDF" }
			
			w.Header().Set("Content-Disposition", fmt.Sprintf("attachment; filename=Report_%s.txt", format))
			fmt.Fprintf(w, "Report Type: %s\nSelected Data: %s\n\nStatus: Filtering logic applied.\nDownloading actual %s file...", format, selection, format)
			return
		}
			...
			<body>
				<div class="box">
					<div class="nav">
						...
						<div style="flex:1; text-align:right;"><a href="/?action=logout" style="color:#d93025; text-decoration:none; font-weight:bold;">Logout</a></div>
					</div>

					<div class="export-bar">
						<strong>Report Generator:</strong>
						<input type="text" id="sel" placeholder="e.g. 3-50 (Range) or 3,7,10 (Custom)" style="flex:1; padding:10px; border:1px solid #ddd; border-radius:5px;">
						<button onclick="window.location.href='/?action=export_excel&selection='+document.getElementById('sel').value" class="btn" style="background:#2ecc71;">Export Excel</button>
						<button onclick="window.location.href='/?action=export_pdf&selection='+document.getElementById('sel').value" class="btn" style="background:#e74c3c;">Export PDF</button>
					</div>

					<form method="POST" style="display:flex; gap:10px; margin-bottom:25px; flex-wrap:wrap;">
						<input type="text" name="customerName" id="n" placeholder="Name" required style="flex:1; padding:12px; border-radius:6px; border:1px solid #ddd;">
						<input type="text" name="customerPhone" id="p" placeholder="Phone" style="flex:1; padding:12px; border-radius:6px; border:1px solid #ddd;">
						<input type="email" name="customerEmail" id="e" placeholder="Email" style="flex:1; padding:12px; border-radius:6px; border:1px solid #ddd;">
						<input type="text" name="customerRemarks" id="r" placeholder="Remarks" style="flex:1; padding:12px; border-radius:6px; border:1px solid #ddd;">
		...
		for rows.Next() {
			...
			fmt.Fprintf(w, `<tr>
				...
				<td>
					<a href="#" style="color:#1a73e8; text-decoration:none;" onclick="document.getElementById('n').value='%s'; document.getElementById('p').value='%s'; document.getElementById('e').value='%s'; document.getElementById('r').value='%s'; document.getElementById('eid').value='%d'; document.getElementById('mb').innerText='Update'; return false;">Edit</a> | 
					<a href="/?action=delete&id=%d" style="color:#d93025; text-decoration:none;">Delete</a>
